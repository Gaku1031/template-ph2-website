...
$answer = $pdo->query("SELECT name FROM choices where valid=1")->fetchAll(PDO::FETCH_ASSOC);
$answer_valid = $pdo->query("SELECT valid FROM choices")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
  ...
  <title>ITクイズ | POSSE 初めてのWebサイト制作</title>
  <!-- スタイルシート読み込み -->
...
<body>
  ...
  <main class="l-main">
    ...
    <div class="p-quiz-container l-container" id="js-quizContainer">
      ...
      <!-- a0~a15まで3飛ばしで生成 -->
      <?php
      for ($a = 0; $a <= count($choices) - 3; $a = $a+3) { ?>
      <?php ${"a" . $a} = array_slice($choices, $a, 3);?>
      <?php } ?>

      <!-- choicesのvalidを3つずつ区切って取得 -->
      <?php
      for ($b = 0; $b <= count($choices) - 3; $b = $b+3) { ?>
      <?php ${"b" . $b} = array_slice($answer_valid, $b, 3);?>
      <?php } ?>
      ...
      <section class="p-quiz-box js-quiz" data-quiz="${questionNumber}">
        <div class="p-quiz-box__question">
          <h2 class="p-quiz-box__question__title">
            ...
            <span
              class="p-quiz-box__question__title__text"><?= $questions[$i]["content"]; ?></span>
          </h2>
        ...
        <div class="p-quiz-box__answer">
          ...
          <ul class="p-quiz-box__answer__list">
            <li class="p-quiz-box__answer__item">
              <!-- pは0~2までfor文でループする -->
              <!-- a0~a15は各要素3つの値が入っているのでcount($a0)は3を表す -->
              <?php for ($p = 0; $p <= count($a0) - 1; $p++) { ?>
                      <button class="p-quiz-box__answer__button js-answer" data-answer="<?php $i ?>" data-correct="<?php print_r(${"b" . $i*3}[$p]["valid"])?>">
                        <!-- aの係数を0~15まで3飛ばしで指定する -->
                        <!-- 0,3,6,9,12,15 -->
                        <!-- a0~a15まで3飛ばしでループして、iと同じタイミングでループしないといけないので$i*3とすることで$a0[0]~$a0[2],$a3[0]~$a3[2]のようにループさせることができる -->
                              <?php print_r (${"a".$i*3}[$p]["name"]) ?>
          ...
          <?php 
          $noteHtmlCite1 = "<cite class='p-quiz-box__note'>";
          $noteHtmlCite2 = "</cite>";
          $noteHtmli = "<i class='u-icon__note'></i>";
          $note = $questions[$i]["supplement"];
          $note ? print $noteHtmlCite1.$noteHtmli.$note.$noteHtmlCite2 : '' ?>
      </section>
      <?php } ?>
    </div>
